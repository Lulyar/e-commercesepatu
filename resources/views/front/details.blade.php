...
<html>
  ...
  <body>
    ...
    >
      <div
        id="top-bar"
        class="flex justify-between items-center px-4 mt-[30px]"
      >
          ...
          <div class="swiper-wrapper">
            ...
            @foreach ($shoe->photos as $itemPhoto)
              ...
              >
                ...
                <img
                src="{{ Storage::url($itemPhoto->photo) }}"
      ...
      <div id="brand" class="flex items-center gap-4 px-4">
        <div class="w-[70px] h-[70px] rounded-[20px] bg-white overflow-hidden">
          <img
            src="{{Storage::url($shoe->brand->logo)}}"
            class="w-full h-full object-contain"
            alt="brand logo"
          />
        </div>
        <div class="flex flex-col">
          ...
          <div class="flex items-center gap-1">
            ...
            <img
              src="{{asset('assets/images/icons/arrow-left.svg')}}"
              class="w-5 h-5"
              alt="icon"
            />
          </div>
      ...
      <form action="{{ route('front.save_order', $shoe->slug) }}" method="POST" class="flex flex-col gap-3">
        ...
        <div
          id="form-bottom-nav"
          class="relative flex h-[100px] w-full shrink-0 mt-5"
        >
          <div class="fixed bottom-5 w-full max-w-[640px] z-30 px-4">
            <div
              class="flex items-center justify-between rounded-full bg-[#2A2A2A] p-[10px] pl-6"
            >
              ...
              <button
                type="submit"
                class="rounded-full p-[12px_20px] bg-[#C5F277] font-bold"
              >
                Buy Now
              </button>
            </div>
